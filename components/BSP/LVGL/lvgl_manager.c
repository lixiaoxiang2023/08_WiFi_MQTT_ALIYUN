...
static lv_obj_t *ui_create_field(lv_obj_t *parent,
                                 const char *title,
                                 lv_coord_t x,
                                 lv_coord_t y,
                                 lv_coord_t w)
...
static void ui_create_boot_page(void)
{
    ...
    lv_label_set_text(title, "Android Style Boot");
    lv_obj_set_style_text_font(title, &lv_font_montserrat_20, 0);
    lv_obj_set_style_text_color(title, lv_color_hex(0x1A4FD8), 0);
    lv_obj_align(title, LV_ALIGN_TOP_LEFT, 0, 0);

    sub = lv_label_create(hero);
    lv_label_set_text(sub, "Power-on initialization");
    lv_obj_set_style_text_font(sub, &lv_font_montserrat_12, 0);
    lv_obj_set_style_text_color(sub, lv_color_hex(0x56637A), 0);
    lv_obj_align_to(sub, title, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 8);

    spinner = lv_spinner_create(hero, 1200, 60);
    ...
    lv_obj_set_width(g_boot_status, 244);
    ...
    lv_obj_align(g_boot_status, LV_ALIGN_BOTTOM_MID, 0, 0);
}
...
static void ui_create_home_page(void)
{
    ...
    title = lv_label_create(hero);
    lv_label_set_text(title, "Wireless U Disk");
    lv_obj_set_style_text_font(title, &lv_font_montserrat_20, 0);
...
static void ui_create_wifi_page(void)
{
    ...
    card = ui_create_card(page, 296, 172);
    lv_obj_align(card, LV_ALIGN_TOP_MID, 0, 0);

    title = lv_label_create(card);
    lv_label_set_text(title, "WiFi Mode");
    lv_obj_set_style_text_font(title, &lv_font_montserrat_18, 0);
    lv_obj_set_style_text_color(title, lv_color_hex(0x182033), 0);
    lv_obj_align(title, LV_ALIGN_TOP_LEFT, 0, 0);

    g_wifi_mode_value = ui_create_field(card, "Mode", 0, 30, 120);
    g_wifi_ssid_value = ui_create_field(card, "SSID", 0, 82, 152);
    g_wifi_connect_value = ui_create_field(card, "Connection", 156, 30, 120);
    g_wifi_ip_value = ui_create_field(card, "STA IP", 156, 82, 120);
    g_wifi_web_ip_value = ui_create_field(card, "Web Server IP", 0, 134, 200);
}
...
static void ui_create_download_page(void)
{
    ...
    g_download_file_value = ui_create_field(card, "File", 0, 28, 216);
    g_download_size_value = ui_create_field(card, "File Size", 0, 72, 120);
    g_download_speed_value = ui_create_field(card, "Speed", 150, 72, 120);
    g_download_eta_value = ui_create_field(card, "ETA", 0, 116, 120);
    g_download_percent_value = ui_create_field(card, "Progress", 150, 116, 120);

    g_download_status_value = lv_label_create(card);
    lv_obj_set_width(g_download_status_value, 132);
    ...
    lv_obj_align(g_download_status_value, LV_ALIGN_TOP_RIGHT, 0, 4);

    g_download_bar = lv_bar_create(card);
    lv_obj_set_size(g_download_bar, 268, 8);
    lv_obj_align(g_download_bar, LV_ALIGN_BOTTOM_MID, 0, 0);
