...
"</div>"
"<div class='tip'>如果产品列表为空，请按设备上的 KEY0 按钮同步数据</div>"
"<div id='modelBox'>"
"<label>平台</label>"
...
"if(!selectedOption){"
"document.getElementById('modelBox').innerHTML = '<div class=\"tip\" style=\"color:red\">请先同步产品列表（按 KEY0）或从下拉列表选择产品</div>';"
"return;"
"}"
"const productId = selectedOption.getAttribute('data-id');"
"if(!productId){"
"document.getElementById('modelBox').innerHTML = '<div class=\"tip\" style=\"color:red\">产品列表数据不完整，请重新按 KEY0 同步</div>';"
"return;"
"}"
...
"}"
"}catch(e){ console.warn('获取产品列表失败(网络问题或数据未同步，请按KEY0):', e); }"
"}"
